    ...
    updateParticle: (particle: Particle, entity: any, context: CanvasRenderingContext2D, deltaTime: number) => {
      ...
      if (currentTime - entity.lastEmitTime[particle.name || ''] >= emitInterval) {
        ...
        const newParticle: Particle = {
          ...particle,
          localposition: {
            x: Number(particle.localposition?.x || 0) + (Math.random() < 0.5 ? -1 : 1) * Math.random() * Number(particle.spread?.x || 0),
            y: Number(particle.localposition?.y || 0) + (Math.random() < 0.5 ? -1 : 1) * Math.random() * Number(particle.spread?.y || 0)
          },
          velocity: {
          ...
          gravity: particle.gravity ? { ...particle.gravity } : { x: 0, y: 0 },
          weather: typeof particle.weather === 'object' ? { ...particle.weather } : 'none'
        };

        if (entity.particleArrays[particle.name || ''].length >= (particle.amount || 1)) {
          entity.particleArrays[particle.name || ''].shift();
        }

        entity.particleArrays[particle.name || ''].push(newParticle);
        entity.lastEmitTime[particle.name || ''] = currentTime;
      }
      ...
      for (let i = particles.length - 1; i >= 0; i--) {
        ...
        if (p.velocity && p.gravity) {
          ...
          const maxVelocity = {
            x: (particle.velocity?.x || 0) + (windSpeed * 0.2),
            y: (particle.velocity?.y || 0) + (windSpeed * 0.2)
          };
        ...
        const centerX = entity.position.x - (p.size / 2);
        const centerY = entity.position.y - (p.size / 2);
